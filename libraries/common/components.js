...
prx.types.tooltip = {
	...
	,onDisplay: function(item,containerid,symbol) {
		...
		cR += '<div class="tooltip-text liveUpdate-textColor-color changeProperty-textColor changeProperty-textFont"> <span data-editableproperty="text">'+prx.componentsHelper.getProp(item.text,'text-textarea')+'</span></div>'
		cR += '</div>';
		cR += '</div>';
...
prx.types.datetime = {
    ...
    , onDisplay: function (item, containerid, symbol) {
        ...
        cR += '<div id="datetime" class="liveUpdate-textColor-color changeProperty-textColor"></div>';
        cR += '</div>';
        cR += prx.items.getComponentAppendDivs(item, containerid, symbol);
        cR += '</div>';
